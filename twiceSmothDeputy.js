...
(function()
{
    ...
    function drawLines(points, ctx, splitRate)
    {
        let i, len = points.length, threeP, fourP, result = []
        ...
        if(len == 3 && points[0] != points[len-1])
        {
            threeP = [points[0],points[1],points[2]]
            result.push(quadratic(threeP, ctx, splitRate, "threeP"))
            ctx.stroke()
            return result
        }

        //常规情况
        if(points[0] != points[len-1])      //非闭合区域
        {
            //非闭合处理方式一：将首尾两段折线变为二次函数，中间的变为三次函数
            threeP = [points[0], points[1], points[2]]
            result.push(quadratic(threeP, ctx, splitRate, "head"))
            for(i = 0;i<len-3;i++)
            {
                fourP = [points[i], points[i+1], points[i+2], points[i+3]]
                result.push(cubic(fourP, ctx, splitRate))
            }
            threeP = [points[len-3], points[len-2], points[len-1]]
            result.push(quadratic(threeP, ctx, splitRate, "tail"))
            //非闭合处理方式二：在首段之前复制一个起点，在尾段复制一个终点
            //再从头到尾做三次函数（猜测最终结果应该与方式一相同，只是表达方式不同）
        }
        else                                //闭合区域
        {
            fourP = [points[len-2], points[0], points[1], points[2]]
            result.push(cubic(fourP, ctx, splitRate))
            for(i = 0;i<len-3;i++)
            {
                fourP = [points[i], points[i+1], points[i+2], points[i+3]]
            ...
            fourP = [points[len-3], points[len-2], points[len-1], points[1]]
            result.push(cubic(fourP, ctx, splitRate))
        }
        ctx.stroke()
        return result
    ...
    twiceSmoth = 
    {
        smoth:function(targets, allObj, ctx, splitRate)
        {
            ...
            for(i = 0;i<targets.length;i++)
            {
                ...
                {
                    points = []
                    for(m = 0;m<allIdx[j].length;m++)
                    {
                        points.push(vertices[allIdx[j][m]])
                    }
                    drawLines(points, ctx, splitRate)
                }
            }
        }
    }
